<script lang="ts">
	...
	async function handleSubmit(event: Event) {
		...
		try {
			...
			if (response.success) {
				toast.success('Game Updated', `"${formData.name}" has been updated successfully.`);
				goto('/admin?tab=games');
			} else {
				toast.error('Update Failed', response.error || 'Failed to update game');
			}
		} catch (error) {
...
<svelte:head>
	<title>Edit Game - Admin - Azurite</title>
</svelte:head>
		...
		{:else if game}
			...
			<div class="mb-8">
				...
				<div class="flex items-center space-x-3">
					...
					<div>
						<h1 class="text-3xl font-bold text-text-primary">Edit Game</h1>
						<p class="text-text-secondary mt-1">Update game information</p>
					</div>
				</div>
			</div>

	...
	onMount(async () => {
		...
		try {
			...
			if (response.success && response.data) {
				...
				errorMessage = response.error || 'Failed to authenticate with Google.';
				isProcessing = false;
				toast.error('Authentication Failed', errorMessage);
				setTimeout(() => goto('/auth/login'), 3000);
			}
...
<svelte:head>
	<title>Google Authentication - Azurite</title>
</svelte:head>
...
<div class="min-h-screen flex items-center justify-center bg-background-primary">
	<div class="text-center">
		{#if isProcessing}
			<Loading size="lg" />
			<h2 class="mt-6 text-2xl font-bold text-text-primary">Authenticating with Google...</h2>

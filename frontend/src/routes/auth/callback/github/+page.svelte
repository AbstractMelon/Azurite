<script lang="ts">
	...
	onMount(async () => {
		...
		// Handle OAuth error
		if (error) {
			errorMessage = 'GitHub authentication was cancelled or failed.';
			isProcessing = false;
		...
		try {
			const response = await authApi.handleOAuthCallback('github', code, state);

			if (response.success && response.data) {
				// Store the authentication data
				const authData = response.data as AuthResponse;
				auth.login(authData.user, authData.token);
				toast.success('Welcome!', 'You have been successfully authenticated with GitHub.');
				
				// Redirect to home page
				goto('/');
			} else {
				errorMessage = response.error || 'Failed to authenticate with GitHub.';
				isProcessing = false;
				toast.error('Authentication Failed', errorMessage);
				setTimeout(() => goto('/auth/login'), 3000);
			}
...
<svelte:head>
	<title>GitHub Authentication - Azurite</title>
</svelte:head>
...
<div class="min-h-screen flex items-center justify-center bg-background-primary">
	<div class="text-center">
		{#if isProcessing}
			<Loading size="lg" />
			<h2 class="mt-6 text-2xl font-bold text-text-primary">Authenticating with GitHub...</h2>

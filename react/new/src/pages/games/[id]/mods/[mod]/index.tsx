      ...
      <div className={styles.container}>
        <div className={styles['mod-details']}>
          <div className={styles['mod-header']}>
            <img src={mod.modIcon} alt={`${mod.name} Icon`} className={styles['mod-icon']} />
            <div className={styles['mod-meta']}>
              <h1>{mod.name}</h1>
              <p className={styles.author}>By {mod.author}</p>
              {mod.teamMembers.length > 0 && (
                <p className={styles.team}>Team: {mod.teamMembers.filter(Boolean).join(', ')}</p>
              )}
              <p className={styles.version}>Version: {mod.version}</p>
              <p className={styles.updated}>Last Updated: {mod.updated}</p>
            </div>
          </div>
          ...
          <Link href={mod.modFile} className={styles['download-button']}>
            Download
          </Link>
        </div>
      </div>
